...
public class UserService {
    ...
    public UserEntity registerUser(UserForm userForm) {
        ...
        if (affiliationId == null) {
            ...
            affiliationRepository.insert(affiliation); // DB保存後、自動採番IDがセットされる
            affiliationId = affiliation.getId(); // 発行されたLong型のIDを取得
        }
        ...
        if (positionId == null) {
            ...
            positionRepository.insert(position); // DB保存後、自動採番IDがセットされる
            positionId = position.getId(); // 発行されたLong型のIDを取得
        }
